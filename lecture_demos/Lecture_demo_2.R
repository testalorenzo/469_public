...
dat <- read.csv("https://github.com/testalorenzo/469_public/blob/main/hw2/framingham.csv/?raw=true")
head(dat)
...
loadings <- pca_res$rotation # this matrix actually contains the eigenvectors of the correlation matrix (loadings)
...
# Loadings are unit vectors (length 1), while scores are much larger. 
# We scale the arrows so they extend toward the edges of your plot.
scaling_factor <- max(abs(pca_res$x[, 1:2])) * 0.7

# 4. Add the arrows
# arrows(x0, y0, x1, y1, ...)
arrows(0, 0, loadings[, 1] * scaling_factor, loadings[, 2] * scaling_factor, 
       col = "red", length = 0.1, lwd = 1.5)

# Add the variable names
# We multiply by 1.2 to nudge the text slightly past the arrow head
text(loadings[, 1] * scaling_factor * 1.15, loadings[, 2] * scaling_factor * 1.15, 
     labels = rownames(loadings), col = "red", font = 2, cex = 0.8)
...
# PC1 represents the direction that maximizes variance
