...
```{r}
source("https://raw.githubusercontent.com/testalorenzo/469_public/master/hw2/hw2_functions.R")
```
load in data, basic exploration
```{r}
dat <- read.csv("https://raw.githubusercontent.com/testalorenzo/469_public/master/hw2/framingham.csv")
dim(dat)
colnames(dat)
head(dat)
summary(dat)
str(dat)
```
for datasets like these (unlike SNP data), it's in your best interest to rescale the data. you don't need to do this step in your homework (we did it for you). Remove education since it's categorical with more than 2 levels
...
to compute the misclassification rate, simply compute 1 - sum the diagonal terms, divided by the grand sum
```{r}
...
let us first try to regress HeartRate onto TotChol, Age, SysBP, DiaBP, CigPDay, BMI and Glucose.
```{r}
...
for(i in 1:2){
  ...
  lines(rep(abs(log.lambda), 2), c(-1e3, 1e3), lwd = 2, lty = 2)
}
```
...
```
Note: this is VERY finicky. you have to convert newx into a matrix, and the matrix has to have exactly the same number of columns as the number of variables within `cvglmnet_res`.
```{r}
head(pred_vec1)
